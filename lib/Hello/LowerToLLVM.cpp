...
public:
  ...
  //上面的这个hello::PrintOp::getOperationName()表示pattern需要匹配的Op名称，也就是说只有该名字的Op才需要进行matchAndRewrite
  //但是如果不指定的话，则需要匹配任何类型。

  //这里是把之前保留的printOp转换成LLVM dialect中的printOp，hello::PrintOp::getOperationName()获取了之前保留的mlir中的op_name
...
void HelloToLLVMLoweringPass::runOnOperation() {
  ...
  //这里执行了全转换，将传入的所有op都转换为LLVM dialect中的op
  if (failed(applyFullConversion(module, target, std::move(patterns)))) {
    signalPassFailure();
  }
  //而第三种转换方式为AnalysisConversion，记录operation转换是否成功，并确定哪些operations可以合法化
  //if (mlir::failed(mlir::applyAnalysisConversion(module, target, std::move(patterns)))) {
    //signalPassFailure();
  //}
}
